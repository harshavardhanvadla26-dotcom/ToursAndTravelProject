  ...
  const updateLocation = async (updatedLocation) => {
    try {
      ...
      const updatedTour = normalizeTour(
        getThunkData(updatedTourResponse, "Failed to refresh tour details")
      );
      setTour(updatedTour);
      setTourImages(updatedTour.tourImages);
    ...
    const fetchTourData = async () => {
      try {
        ...
        const tourData = normalizeTour(
          getThunkData(response, "Failed to load tour details")
        );
        setTour(tourData);

        if (tourData.tourImages.length > 0) {
          setTourImages(tourData.tourImages);
        }
      } catch (error) {
