...
import { allTickets, getThunkData } from "../../../Redux/API/API";
...
const TicketSummaryComponent = () => {
  ...
  useEffect(() => {
    const fetchTicketSummary = async () => {
      try {
        ...
        const data = getThunkData(response, "Failed to fetch ticket summary");
        setTicketSummary(Array.isArray(data.summary) ? data.summary : []);
      } catch (err) {
        setError(err.message || "Failed to fetch ticket summary");
      } finally {
        setLoading(false);
      }
    };
  ...
  const totalTicketsSold = ticketSummary.reduce(
    (sum, tour) => sum + (tour.ticketsSold || 0),
    0
  );
  const totalTicketsAvailable = ticketSummary.reduce(
    (sum, tour) => sum + (tour.ticketsAvailable || 0),
    0
  );
  const totalRevenue = ticketSummary.reduce(
    (sum, tour) => sum + (tour.totalRevenue || 0),
    0
  );
