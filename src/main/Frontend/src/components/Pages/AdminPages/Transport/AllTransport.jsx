...
import { adminTransport, getThunkData } from "../../../../Redux/API/API";
...
const AllTransport = () => {
  ...
  useEffect(() => {
    dispatch(adminTransport())
      .then((response) => {
        const data = getThunkData(response, "Error fetching transport data");
        setTransports(Array.isArray(data) ? data : []);
      })
      .catch((err) => {
        setError(err.message || "Error fetching transport data");
        console.error(err);
      })
      .finally(() => {
        setLoading(false);
