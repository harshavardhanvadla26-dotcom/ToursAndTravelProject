...
import { adminLodging, getThunkData } from "../../../../Redux/API/API";
  ...
  const renderStars = (rating) => {
    return [...Array(5)].map((_, index) => (
      <Star
        key={index}
        className={`w-4 h-4 ${
          index < Math.round(rating || 0) ? "text-yellow-400" : "text-gray-300"
        }`}
        fill={index < Math.round(rating || 0) ? "#fbbf24" : "none"}
      />
    ));
  };
      ...
      <div className="space-y-2">
        ...
        <div className="flex justify-between items-center mt-3">
          <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded">
            Rating: {(lodging.rating || 0).toFixed(1)}
          </span>
        </div>
      </div>
    </div>
...
const AllLodging = () => {
  ...
  useEffect(() => {
    dispatch(adminLodging())
      .then((response) => {
        const data = getThunkData(response, "Error fetching lodging data");
        setLodgings(Array.isArray(data) ? data : []);
      })
      .catch((err) => {
        setError(err.message || "Error fetching lodging data");
        console.error(err);
      })
      .finally(() => {
        setLoading(false);
