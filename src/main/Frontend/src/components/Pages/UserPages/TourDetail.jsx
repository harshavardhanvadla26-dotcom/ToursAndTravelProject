  ...
  useEffect(() => {
    const fetchTourData = async () => {
      try {
        ...
        const responseData = getThunkData(response, "Failed to load tour details");
        const tourDetails = normalizeTour(responseData.tourDetails);
        if (!tourDetails) {
          throw new Error("Tour not found");
        }
        setTour(tourDetails);

        if (tourDetails.tourImages.length > 0) {
