...
public interface BookingRepo extends JpaRepository<Booking, Long> {
    ...
    @Query("SELECT DISTINCT t FROM Tour t " +
            "LEFT JOIN t.location location " +
            "LEFT JOIN t.lodging lodging " +
            "LEFT JOIN t.transport transport " +
            "WHERE (:country IS NULL OR location.country = :country) AND " +
            "(:lodgingType IS NULL OR lodging.lodgingType = :lodgingType) AND " +
            "(:transportType IS NULL OR transport.transportType = :transportType) AND " +
            "(:minPrice IS NULL OR t.price >= :minPrice) AND " +
            "(:maxPrice IS NULL OR t.price <= :maxPrice)")
    List<Tour> filterTours(@Param("country") String country,
