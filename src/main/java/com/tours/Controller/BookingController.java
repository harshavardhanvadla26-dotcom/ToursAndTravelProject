...
public class BookingController {
    ...
    private boolean isStripeConfigured() {
        return stripeSecretKey != null && stripeSecretKey.startsWith("sk_");
    }
    ...
    public ResponseEntity<?> createPaymentIntent(
        ...
        try {
            ...
            if (!isStripeConfigured()) {
                String paymentIntentId = "dev-payment-" + preliminaryBooking.getBookingId();
                return ResponseEntity.ok(Map.of(
                        "paymentIntentId", paymentIntentId,
                        "checkoutSessionId", "dev-checkout-" + preliminaryBooking.getBookingId(),
                        "bookingId", preliminaryBooking.getBookingId(),
                        "totalAmount", preliminaryBooking.getTotalPrice(),
                        "checkoutUrl", "http://localhost:5173/success?paymentIntentId="
                                + paymentIntentId + "&bookingId=" + preliminaryBooking.getBookingId()
                ));
            }
        ...
        try {
            ...
            return ResponseEntity.ok(Map.of(
                    ...
                    "booking", Map.of(
                            ...
                            "tourId", confirmedBooking.getTour().getId(),
                            "tourName", confirmedBooking.getTour().getTourName(),
                            "numberOfTickets", confirmedBooking.getNumberOfTickets(),
                            "totalPrice", confirmedBooking.getTotalPrice(),
                            "paymentStatus", confirmedBooking.getPaymentStatus().name(),
                            "bookingConfirmed", confirmedBooking.isBookingConfirmed()
                    )
            ));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
